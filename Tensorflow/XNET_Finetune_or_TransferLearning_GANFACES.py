...
def TrainTwoClass(model, batch_size, epochs, model_dir, class0_dir, class1_dir, labels, augmentation, cutoff=None):
    # Create training and validation sets and labels
    x_train, x_validation, y_train, y_validation = create_training_validation_sets(class0_dir, class1_dir, labels, cutoff)
...
if __name__ == '__main__':
    ...
    exp_id = f"XCeption-GANFACES-from-watermarked-{task}-{'-'.join(model_file.split('/')[2].split('-')[3:])}"
    ...
    TrainTwoClass(model=model,
                  ...
                  class0_dir="datasets/GANFACES/Train/Pristine",
                  class1_dir="datasets/GANFACES/Train/GAN",
                  labels=[0, 1],
                  augmentation=False, cutoff=0.7)
