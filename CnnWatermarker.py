...
from FastCustomModel import CustomModel
from CustomGenerators import create_batch_generators, create_training_validation_sets, augment_on_test, create_cifar_generators
...
def EncodeMessage(key, w_length, msg="Lorem ipsum dolor sit amet conse", msg_length=256):
    ...
    # Create a random antipodal sequence (-1, +1)
    np.random.seed(key)
    watermark = 2 * np.random.randint(2, size=w_length).flatten() - 1

    # Modulate the random sequence with the spread binary message
    return np.repeat(2*np.array(bits) - 1, spread_factor) * watermark, np.array(bits)
...
def target_layer_exists(hlayer, bmodel):
    ...
    if bmodel == 'exception':
        ...
        return len([x for x in tf.keras.applications.VGG16(weights=None).layers if x.name == hlayer])
    elif bmodel == 'efficientnet':
        return len([x for x in tf.keras.applications.EfficientNetB0(weights=None).layers if x.name == hlayer])
...
def get_target_layer(hlayer, bmodel):
    ...
    if bmodel == 'vgg':
        return [x for x in tf.keras.applications.VGG16(weights=None).layers if x.name == hlayer]
    if bmodel == 'efficientnet':
        return [x for x in tf.keras.applications.EfficientNetB0(weights=None).layers if x.name == hlayer]
...
class CnnWatermarker:
    ...
    def SetCustomModel(self, model, input_shape=None, num_classes=None):
        ...
        # If the number of output nodes is different than the oProne from the loaded model, then change output size
    ...
    def ExtractMultibitWatermark(self, orig_watermark, message_bits, host_layers, spread_factor=None):
        ...
        _, data = self.ExtractWatermark(host_layers=host_layers, watermark_length=len(orig_watermark))
        hosts, _ = zip(*data)

        # Take the signum of the hosts
        ...
        for c in range(0, len(message_bits)):
            ...
            orig_chunk = (2 * message_bits[c] - 1) * orig_watermark[c * spread_factor:(c + 1) * spread_factor]

            d1 = np.dot(rec_chunk, orig_chunk)
            d2 = np.dot(rec_chunk, -orig_chunk)

            decision = 0 if d1 < d2 else 1

            rec_message.append(int(decision))
    ...
    def CreateWatermark(self, watermark, watermark_length, strength_distribution, host_layers):
        ...
        # Generate a random watermark if one was not provided as input
        if watermark is None:
            print(f'A watermark was not provided. Generating random watermark with length {watermark_length}')
            watermark = 2 * np.random.randint(2, size=self._watermark_length).flatten() - 1
    ...
    def Test(self, model_path, class0_dir, class1_dir, labels, augmentation=False, output_dir='./', log_file='log.txt'):
        ...
        with open(os.path.join(output_dir, log_file), 'a+') as f:
            utils.print_redirect(f, f"Test Accuracy: {accuracy}")
            utils.print_redirect(f, 'Confusion Matrix: ')
            utils.print_redirect(f, confusion_matrix(test_labels, pred_labels))

        return accuracy, (test_images, test_labels, pred_labels, predictions)

    def TrainGtsrdb(self, batch_size, epochs, model_dir, gtsrb_dir, output_dir, run_eagerly=False):

        os.makedirs(output_dir, exist_ok=True)

        (trainX, trainY) = pickle.load(open(os.path.join(gtsrb_dir, 'Train.p'), 'rb'))
        (testX, testY) = pickle.load(open(os.path.join(gtsrb_dir, 'Test.p'), 'rb'))
        ...
        history = self._model.fit(data_aug.flow(trainX, trainY, batch_size=batch_size),
                                  epochs=epochs,
        ...
        with open(os.path.join(output_dir, 'traininghist.p'), 'wb') as file_pi:
            pickle.dump(history.history, file_pi)

        return

    def TestGtsrdb(self, gtsrb_dir, output_dir):
        ...
        # Predict
        predictions = self._model.predict(testX)
    ...
    def TrainCifar(self, batch_size, epochs, model_dir, output_dir, dataset='cifar10', run_eagerly=False):
        ...
        if dataset == 'cifar10':
            ...
            print(f"{dataset} is not a valid dataset name. Use either 'cifar10' or 'cifar1000'")
            return
        ...
        history = self._model.fit(data_aug.flow(x_train, y_train, batch_size=batch_size),
                                  epochs=epochs,
    ...
    def TestCifar(self, output_dir, top_n=3, dataset='cifar10', log_file='log.txt'):
        ...
        predictions = self._model.predict(x_test/255.0)
        ...
        for i in range(len(predictions)):
            ...
            top_values = (-i_pred).argsort()[:top_n]
